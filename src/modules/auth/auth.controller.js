...
export async function loginWithEmail(req, res) {
    try {
        ...
        if (!user) {
            ...
            // user = await User.create({ info: { email }, providers: [{ uid: email, password: bcrypt.hashSync(password, 10), type: provider }] });
        }
        ...
        // bcrypt.compare(password, userProvider.password, (err, isMatch) => {
        //     if (err) throw err;
        //     if (!isMatch) {
        //         res.status(500).json({ message: 'Password did not match.' });
        //     } else {
